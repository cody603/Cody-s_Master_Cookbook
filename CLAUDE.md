...
**So the skill is one paragraph that points at one file, and everything else is in this repo.** Four hand-maintained files at the root, none of them cookbook content:
...
- **`CONSUMABLES.md`** — **the repeat-buy log, added 2026-09-14.** Cody: *"I've loved the Zevia cream sodas and the Zevia root beers… there needs to be a time log or something. Hey, it's been a week, and you've cooked this and this and this — are you low on Zevia? … I think we're missing drinks and the necessity of drinks in our system. Obviously that's kind of separate than the actual cookbook."* Drinks and anything else this house buys on a rhythm, with **when each was last bought**, so a planning conversation can ask instead of waiting to be told. **This is the second file chat may write, and it needs no approval** — a *last bought* date stamp, or a new item someone says they buy regularly. It is a shopping log, not cookbook content, which is exactly why it is writable; anything that would change a **recipe** is still a revision request. **Two honest limits are written into the file and must stay there:** the intervals are observed, not prescribed *(his "about a week" for the Zevia is his own figure, and it supersedes anything estimated)*, and **nothing here can reach out on its own** — a chat session has no background process, so the question only gets asked when somebody opens a conversation.
