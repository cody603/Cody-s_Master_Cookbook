...
**Used by:** the chat side (`CHAT.md` §7) whenever someone is cooking; the *tonight's instructions* document; [§T111](t111-planning-a-week--the-conversation-mode-workflow.md) for the week-ahead version.
...
- **Where it lives — added 2026-09-14: the repo, first.** The document is written to **`tonight/<date-of-dinner>.md`** in this repo the moment it is built, and that file is what the spoken version (§4) reads. Cody: *"it just needs to be easily accessible… with the Word doc I have to actually manually share this thing."* A file in the repo is fetched by URL like every other file the chat uses, so there is nothing to share and nothing to find. **The Word doc and the email are copies on request**, not the mechanism. `tonight/README.md` carries the format; each step that has a technique names its §T planner file in brackets so the chat can fetch it without a lookup.

#### 4. The spoken version — one step, then stop

- **Tonight's document is the sequence — fetch it and follow it.** Cody, 2026-09-14: *"the live cooking in chat is really just following the logic of the word document. The word document is where all the logic is. It's gonna give you steps to start — so maybe you start the caulimash step the same time you start the Brussels sprout sauté."* The spoken walkthrough does not re-derive the order, the pairings or the timing; the document already decided them (§3), and that is the work that needs the stronger model. **The trigger is the file:** when anyone says *let's cook*, *what's first*, *walk me through dinner*, fetch `tonight/<today>.md` and read step one. **Only if that file doesn't exist** does the chat build the merged sequence itself, by §2–§3, before saying the first step — and it says so first. Keep a private pointer the whole time — *tonight: these dishes; we are on this dish, this step* — and never say the step number out loud.
- **Techniques come from the tags.** When a step has a technique in it — *sauté the Brussels sprouts*, *bring the steak to 129*, *make the roux* — the recipe entry's **Techniques used** line and its `TECHNIQUE-TAGS` name the §T entry; fetch that `planner/recipes/t…` file and answer from it. Cody: *"if it's sauté Brussels sprouts, well, then you will know certain techniques, and the chat should be able to retrieve that based on our tagging system."* Retrieve, don't recall.
...
- **Last-minute ingredient swaps and on-the-spot technique changes are expected, and the chat handles them itself — added 2026-09-14.** Cody: *"There will be last-minute adjustments, mark my words, where you may have to have a substitute for coriander, or we don't have whole milk, we have half-and-half only… I don't mind if it's Sonnet or Haiku, because I think those are simple things that you can just look up and change… use your judgment and figure out."* The order for answering: **this cookbook first** — [§T106](t106-salts-stocks--broths-sugars-vinegars-winebeerspirits-and-zest--pantry-reference-the-meathead-method.md) and the dish's own §T entries — **then one quick lookup from a credible source** ([CLAUDE.md §3d](../../CLAUDE.md), the short form: a named chef or test kitchen, never a content farm), **said in one breath with the source named** — *"half-and-half is fine here; it's richer, so ease off the butter — Kenji says the same."* Then: **write the swap into `tonight/<date>.md`** under *Swaps made tonight* and adjust that dish's remaining steps, so the record survives and the cooking log can carry it. **A swap is for tonight only** unless someone says it should stick, and then it is a revision request as well. What the chat must not do is stall: a substitution question in a kitchen gets an answer in the next sentence, with the cookbook's honest confidence attached.
